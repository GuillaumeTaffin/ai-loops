...
- [Detailed Design](loop-detailed-design.md) — a concrete implementation of this architecture as a Claude Code plugin, with skills, custom agents, file-based artifacts, and git as the timeline. As of v0.2.0, the plugin implements hierarchical flow-based execution exclusively.
- [Hierarchical Loops](hierarchical-loops-design.md) — extending the architecture to support hierarchical control loops where an inner loop acts as a composite actuator for an outer loop. This is the active execution model as of v0.2.0.
